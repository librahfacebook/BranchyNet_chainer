...
# 保存已经训练好的网络模型
with open("../models/resnet_cifar10.bn", "wb") as f:
    dill.dump(branchyNet, f)


# 加载已经保存好的模型
# with open("../models/resnet_cifar10.bn","rb") as f:
...
visualize.plot_line_tradeoff(g_accs, g_diffs, g_ts, g_exits, g_baseacc, g_basediff,
                             ...
                             output_path='../pic/resnet_gpu.png')
# 将结果保存为csv文件
utils.branchy_save_csv(g_baseacc, g_basediff, g_accs, g_diffs, g_exits, g_ts, filename='ResNet')

# 显示GPU下网络测试的结果（网络类型、准确率、运行时间、与原始网络的运行效率倍数、
# 退出点阈值和退出样本数比例）
print("GPU Results:")
utils.branchy_table_results(filename='ResNet')
'''